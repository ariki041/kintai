...
<div class="container-fluid">
	...
	<div class="row" id="header">
		<div class="col-sm-3 col-sm-offset-1 col-xs-10">
			<div class="btn-inline">
				<form method="get">
					...
					<span id="year"><!--{$date[0]['year']}--></span>年<span id="month"><!--{$date[0]['month']}--></span>月
		...
		<div class="col-sm-2 col-sm-offset-6 col-xs-2">
			<button class="btn" id="prevPage" onclick="location.href='/kintai/Admin/index'">管理画面へ戻る</button>
		</div>
	</div>
		...
		<table class="table table--extend table-bordered table-condensed table-hover" id="content1">
			...
			<tbody>
					...
					<form method="post" name="syuttaikin">
							...
							<!--{/if}-->
								<td><!--{$date[date]['day']}--></td>
								<td><!--{$date[date]['week']}--></td>
								<td>
									<div <!--{if $validationDate==$date[date]['day']&&isset($this->validationErrors['Work']['syukkin'][0])}-->class="form-group has-error"<!--{/if}-->>
										<!--{if $validationDate==$date[date]['day']}--><label class="control-label"><!--{$this->validationErrors['Work']['syukkin'][0]}--></label><!--{/if}-->
								...
								<td>
									<div <!--{if $validationDate==$date[date]['day']&&isset($this->validationErrors['Work']['taikin'][0])}-->class="form-group has-error"<!--{/if}-->>
										<!--{if $validationDate==$date[date]['day']}--><label class="control-label"><!--{$this->validationErrors['Work']['taikin'][0]}--></label><!--{/if}-->
								...
								<td>
									<div <!--{if $validationDate==$date[date]['day']&&isset($this->validationErrors['Work']['recess_time1'][0])}-->class="form-group has-error"<!--{/if}-->>
										<!--{if $validationDate==$date[date]['day']}--><label class="control-label"><!--{$this->validationErrors['Work']['recess_time1'][0]}--></label><!--{/if}-->
								...
								<td>
									<div <!--{if $validationDate==$date[date]['day']&&isset($this->validationErrors['Work']['recess_time2'][0])}-->class="form-group has-error"<!--{/if}-->>
										<!--{if $validationDate==$date[date]['day']}--><label class="control-label"><!--{$this->validationErrors['Work']['recess_time2'][0]}--></label><!--{/if}-->
								...
								<td><input type="hidden"><!--{html_options name=day_state id="day_state_`$date[date]['day']`" options=$day_state selected=$date[date]['day_name_selected']}--></td>
								<td>
